...
describe('TransactionService', () => {
  ...
  describe('createTransaction', () => {
    it('should create a new transaction', () => {
      ...
      service.createTransaction(newTransaction).subscribe(response => {
        ...
        if (response.symbol) expect(response.symbol).toBe('MSFT');
        if (response.quantity) expect(response.quantity).toBe(20);
      });
  ...
  describe('updateTransaction', () => {
    it('should update an existing transaction', () => {
      ...
      service.updateTransaction(transactionId, updates).subscribe(response => {
        expect(response).toBeDefined();
        if (response.quantity !== undefined) expect(response.quantity).toBe(15);
        if (response.totalAmount !== undefined) expect(response.totalAmount).toBe(2250);
      });

      const req = httpMock.expectOne(`${apiUrl}/${transactionId}`);
  ...
  describe('getTransactionById', () => {
    it('should fetch a single transaction by id', () => {
      const transactionId = 1;

      service.getTransactionById(transactionId).subscribe(response => {
        expect(response).toEqual(mockTransactions[0]);
      });
